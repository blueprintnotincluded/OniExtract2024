...
public class DecompressionGasValveConfig : IBuildingConfig
{
	...
	{
		Effect = "Allows " + UI.FormatAsLink("Gases", "ELEMENTS_GAS") + " to be transfered from " + UI.FormatAsLink("High Pressure Gas Pipe", "HighPressureGasConduit") + " to normal " + UI.FormatAsLink("Pipes", "GASCONDUIT") + ".";
	...
	public override BuildingDef CreateBuildingDef()
	{
		...
		BuildingDef val = BuildingTemplates.CreateBuildingDef("DecompressionGasValve", 2, 1, "deco_gas_valve_kanim", 100, 50f, array, array2, 800f, (BuildLocationRule)0, PENALTY.TIER1, NOISY.TIER0, 0.2f);
		val.Floodable = false;
		val.Overheatable = false;
		val.ViewMode = GasConduits.ID;
		val.AudioCategory = "HollowMetal";
		val.InputConduitType = (ConduitType)1;
		...
		GeneratedBuildings.RegisterWithOverlay(OverlayScreen.GasVentIDs, ((Def)val).PrefabID);
		return val;
	}
	...
	public override void ConfigureBuildingTemplate(GameObject go, Tag prefab_tag)
	{
		...
		val.storageFilters = STORAGEFILTERS.GASES;
		val.capacityKg = 10f;
		...
		val3.conduitType = (ConduitType)1;
